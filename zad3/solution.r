setwd("/home/mateusz/Dokumenty/studia/sus/sus/zad3")
...
predictGLM = function(trainingSet, clsTr, testSet) {
  ...
  # tmp = cbind(testSet, rep(0, nrow(testSet)))
  # tmp[1, colsNo+1] = 1
  tmp = testSet
  h2oDataTest = as.h2o(tmp)
  
  # colnames(h2oDataTest)[colsNo+1] = "clsTr"
...
predictColumn = function(colId) {
  ...
  # predictions = predictXgboostForCols(trainingSet, clsTr, testSet)
...
# ? to NA
# dataSet = as.data.frame(lapply(dataSet, function(y) sapply(y, function(x) if (x == "?") {0} else {x})))
